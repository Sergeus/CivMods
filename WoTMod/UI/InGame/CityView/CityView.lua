...
function OnCityViewUpdate()
	...
	if (pCity ~= nil) then
		...
		if (pCity:IsHasGovernor()) then
			Controls.TitlePanel:SetOffsetVal(0, 132)
			Controls.GovernorPanel:SetHide(false)

			local governorInfo = GameInfo.Governors[pCity:GetGovernorType()]

			IconHookup( governorInfo.IconIndex, 128, governorInfo.IconAtlas, Controls.GovIcon )

			local yieldString = ""

			for yield in GameInfo.Yields() do
				local yieldChange = pCity:GetGovernorYieldChange(yield.ID)
				if (yieldChange > 0) then
					yieldString = yieldString .. yield.IconString .. Locale.ConvertTextKey(yield.Description) .. ": +" .. yieldChange .. "[NEWLINE]"
				elseif (yieldChange < 0) then
					yieldString = yieldString .. yield.IconString .. Locale.ConvertTextKey(yield.Description) .. ": -" .. yieldChange .. "[NEWLINE]"
				end
			end 

			local toolTipString = Locale.ConvertTextKey("TXT_KEY_CITYVIEW_CITY_GOVERNOR", governorInfo.Description,
				yieldString, governorInfo.Help)

			Controls.GovIcon:SetToolTipString(toolTipString)
		else
			Controls.TitlePanel:SetOffsetVal(0, 32)
			Controls.GovernorPanel:SetHide(true)
		end
