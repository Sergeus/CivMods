...
#if WOTMOD
						for (iJ = 0; iJ < GC.GetNumYieldInfos(); ++iJ)
#else
						for(iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
#endif // WOTMOD
						{
...
int CvPlayer::GetYieldRateFromMinorCivs(YieldTypes eYield, CvString* tooltipSink) const
{
	...
	for (int iMinorLoop = MAX_MAJOR_CIVS; iMinorLoop < MAX_CIV_PLAYERS; iMinorLoop++)
	{
		CvPlayerAI& kMinor = GET_PLAYER(static_cast<PlayerTypes>(iMinorLoop));
		if (kMinor.isAlive())
		{
			switch (eYield)
			{
...
int CvPlayer::GetYieldRateFromReligion(YieldTypes eYield, CvString* tooltipSink) const
{
	...
	if(eFoundedReligion != NO_RELIGION)
	{
		...
		if(pReligion)
		{
			iYieldRate += pReligion->m_Beliefs.GetHolyCityYieldChange(eYield);

			int iTemp = pReligion->m_Beliefs.GetYieldChangePerForeignCity(eYield);
			if (iTemp > 0)
			{
				iYieldRate += (iTemp * GetReligions()->GetNumForeignCitiesFollowing());
			}
...
{
#if WOTMOD
	int iHappiness = GetPlayerPolicies()->GetGlobalYieldRate(YIELD_HAPPINESS);
