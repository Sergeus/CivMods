...
aArgastDenouncePlayerTurn = GameInfo.SiegeModConstants.SIEGEMOD_ARGAST_DENOUNCE_PLAYER_NOT_AT_WAR.Value
...
function InitArgastWar(pArgast)
	...
	print("Declaring war...")
	pArgastTeam:DeclareWar(pSvesta:GetTeam())
end
...
function MaintainArgastWar(pArgast)
	...
	if (not pArgastTeam:IsAtWar(pPlayer.GetTeam())) then
		print("The MaintainArgastWar function has been called even though Argast isn't at war with the player...")
		return
	end
...
function ArgastWar(playerID)
	...
	elseif currentTurn > aArgastDenouncePlayerTurn and not pArgastTeam.IsAtWar(pSvesta:GetTeam()) then
